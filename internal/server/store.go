...
CREATE INDEX IF NOT EXISTS idx_resource_chunks_chunk ON resource_chunks(chunk_id);`,
	...
	// 6: per-pack object counters, so GC candidate selection reads the packs table
...
func (s *Store) migrate() error {
	...
	// A legacy layout must fail loud before any step touches it: migration 6's
...
func (s *Store) ResourceVisibility(id string) (api.Visibility, error) {
	var vis string
	err := s.rdb.QueryRow(`SELECT visibility FROM resources WHERE id = ?`, id).Scan(&vis)
	if errors.Is(err, sql.ErrNoRows) {
		return "", ErrNotFound
	}
	if err != nil {
