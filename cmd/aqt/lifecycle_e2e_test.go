...
func (app *application) pullFreshErr(t *testing.T, ref, password string) error {
	t.Helper()
	var perr error
	withFreshEnv(t, func() {
		out := filepath.Join(t.TempDir(), "out.bin")
		perr = app.runPull(ref, out, password, false, false)
	})
	return perr
}
...
func TestPushMaxReadsInline(t *testing.T) {
	...
	for i := range 2 {
		if got := app.pullFresh(t, ref, ""); len(got) != len(data) {
			t.Fatalf("pull %d got %d bytes, want %d", i, len(got), len(data))
		}
	}
	if err := app.pullFreshErr(t, ref, ""); !errors.Is(err, client.ErrGone) {
...
func TestShareExpireAfterFact(t *testing.T) {
	...
	shareRef := strings.TrimSpace(captureStdout(t, func() {
		if err := app.runShare(id, "", true, linkPolicy{expireSeconds: 3600}); err != nil {
			t.Fatalf("share: %v", err)
		}
	}))
	// Sanity: it is pullable before the deadline.
	if got := app.pullFresh(t, shareRef, ""); string(got) != "shared with a deadline" {
	...
	if err := app.pullFreshErr(t, shareRef, ""); !errors.Is(err, client.ErrGone) {
