...
// syncSession holds local state and unlocked credentials for one sync.
...
func (app *application) openSyncSession(root string, opts syncOptions) (syncSession, error) {
	st, err := folderstate.LoadState(root)
	if err != nil {
		return syncSession{}, err
	}
	base, baseExists, err := folderstate.LoadBaseForSync(root, app.profile)
	if err != nil {
		return syncSession{}, err
	}
...
// Wipe zeroes this session's master key; callers must wipe any by-value copies too.
...
// (--reconcile) or when an accepted rollback invalidated it.
...
// regressed reports that before an unsupported-format error.
